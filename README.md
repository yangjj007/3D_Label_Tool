...
本项目是一个**创新的3D模型智能标注工具**，核心功能是**自动化细粒度语义标签生成**和**用户友好的可视化数据审查和编辑**。不同于传统的手动标注或简单的几何标签，本系统能够：

### 🤖 智能语义理解
- **多视角渲染分析**：自动从多个视角（轴向、冠状、矢状）渲染3D模型为2D图像
- **结构化标签生成**：为模型的每个材质（Mesh）生成富含语义的文本描述，包括部件名称、功能说明、空间关系等

### ⚡ 高效批量处理
- **智能并发控制**：支持多任务并发处理，配合速率限制和错误重试机制，确保稳定性
- **断点续传**：支持中断恢复，节省计算资源

### 📝 标签持久化与编辑
- **GLB/GLTF原生支持**：语义标签直接写入模型文件的`extras.semanticLabel`字段，无需额外文件管理
- **在线可视化编辑**：提供图形化界面，可选中材质对象实时查看、编辑和更新语义标签
- **多级存储机制**：IndexedDB本地缓存 + 服务器持久化，确保数据安全

### 🎨 应用场景
- **3D资产管理**：为大规模3D模型库自动生成可搜索的语义元数据
- **医学影像标注**：对医学3D模型（CT、MRI重建）进行解剖结构自动标注
- **工业设计**：为CAD模型的零部件生成功能描述和装配关系说明
- **游戏开发**：批量为游戏资产添加语义标签，辅助资产检索和自动化处理
---
